...
stList *bubbleGraph_getProfileSeqs(BubbleGraph *bg, stReference *ref) {
	...
	stHash *readLengths = stHash_construct2(NULL, free);
	...
	for(uint64_t i=0; i<bg->bubbleNo; i++) {
		...
		for(uint64_t j=0; j<b->readNo; j++) {
			...
			uint64_t *k = stHash_search(readLengths, s->read);

			if(k == NULL) { // We are starting a new read, so make a new
				// length entry
				k = st_calloc(1, sizeof(uint64_t));
				stHash_insert(readLengths, s->read, k);
			}

			*k += 1; // Increment length
		}
	}
	...
	for(uint64_t i=0; i<bg->bubbleNo; i++) {
		...
		for(uint64_t j=0; j<b->readNo; j++) {
			...
			if(pSeq == NULL) { // We are starting a new read, so make a new
				...
				uint64_t *k = stHash_search(readLengths, s->read);
				assert(k != NULL);
				assert(i+*k < bg->bubbleNo);
				pSeq = stProfileSeq_constructEmptyProfile(ref, s->read->readName, i, i+*k);
			...
			float maxLobProb = b->alleleReadSupports[b->alleleNo * j];
			for(uint64_t k=1; k<b->alleleNo; k++) {
				if(b->alleleReadSupports[b->alleleNo * j + k] > maxLobProb) {
					maxLobProb = b->alleleReadSupports[b->alleleNo * j + k];
				}
			}
			...
			for(uint64_t k=0; k<b->alleleNo; k++) {
				int64_t l = b->alleleReadSupports[b->alleleNo * j + k] - maxLobProb;
				assert(l >= 0);
...
stReference *bubbleGraph_getReference(BubbleGraph *bg) {
	...
	ref->referenceName = stString_copy(bg->refString);
...
stGenomeFragment *bubbleGraph_phaseBubbleGraph(BubbleGraph *bg, Params *params) {
	...
	stReference *ref = bubbleGraph_getReference(bg);
